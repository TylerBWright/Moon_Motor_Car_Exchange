...
input[type="submit"] {
	...
	cursor: pointer;
			...
			<br>
			<p>Please note, only vehicles in a pending transaction status are displayed below. To review a vehicle still in the listings, head over to the buy page.</p>
			<br>
			<h2>Display all vehicles pending sale:</h2>
			<br>
			
			<form action="/pendingtransseller" method="get">
			...
			<br></br>
			<h2>Display all vehicles pending purchase:</h2>
			<br>
	
			<form action="/pendingtransbuyer" method="get">
			...
			<c:choose>
				<c:when test="${not empty carsPending}">
					<c:forEach var="item" items="${carsPending}">
						...
						<br>
						<form:form action="/cancelCar?number=${item.number}">
